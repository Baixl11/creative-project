from __future__ import annotations

import re

from app.models import MatchReport, RequirementEvidence


class MatchEngine:
    STOP_WORDS = {
        "a",
        "an",
        "and",
        "are",
        "as",
        "at",
        "be",
        "by",
        "for",
        "from",
        "in",
        "into",
        "is",
        "of",
        "on",
        "or",
        "the",
        "to",
        "with",
        "will",
        "your",
    }

    SYNONYM_GROUPS = (
        {
            "stakeholder",
            "stakeholders",
            "cross-functional",
            "alignment",
            "align",
            "aligned",
            "communicate",
            "communication",
            "priority",
            "priorities",
            "updates",
        },
        {"data", "analytics", "analysis", "dashboard", "dashboards", "metric", "metrics", "sql"},
        {"product", "roadmap", "roadmaps", "prioritization", "prd", "requirements"},
        {"experiment", "experiments", "testing", "ab", "activation", "retention"},
        {"ai", "llm", "llm-powered", "automation", "agent"},
        {"design", "designer", "designers"},
        {"engineering", "engineer", "engineers"},
        {
            "ai",
            "ai产品",
            "人工智能",
            "智能",
            "智能化",
            "智能化产品",
            "大模型",
            "模型",
            "算法",
            "自动化",
        },
        {"g端", "政务", "政务产品", "政府", "企业级", "b端", "to b"},
        {
            "产品",
            "产品规划",
            "规划",
            "产品设计",
            "设计",
            "产品定义",
            "产品方案",
            "产品功能",
            "产品特性",
            "功能设计",
            "0-1",
            "0 到 1",
            "从 0 到 1",
        },
        {
            "技术",
            "技术团队",
            "研发",
            "工程",
            "工程师",
            "协同",
            "合作",
            "协作",
            "跨部门",
            "协调资源",
            "推进项目",
            "推动",
            "落地",
            "实现",
            "优化",
        },
        {
            "市场",
            "市场需求",
            "市场趋势",
            "市场洞察",
            "市场变化",
            "产品定位",
            "策略",
            "策略支持",
            "创新",
        },
        {
            "用户",
            "user",
            "users",
            "用户需求",
            "需求",
            "需求洞察",
            "洞察用户",
            "用户调研",
            "调研",
            "research",
            "客户",
            "customer",
            "customers",
            "客户反馈",
            "feedback",
            "专业用户",
        },
        {
            "结果",
            "业务成果",
            "上线",
            "用户量",
            "使用量",
            "覆盖率",
            "提升",
            "突破",
            "效率",
        },
    )

    def evaluate(self, requirements: list[str], resume_highlights: list[str]) -> MatchReport:
        resume_token_sets = []
        for item in resume_highlights:
            tokens = self._tokenize(item)
            resume_token_sets.append((item, tokens, self._expand(tokens)))

        matched: list[str] = []
        missing: list[str] = []
        rationale: list[str] = []
        requirement_evidence: list[RequirementEvidence] = []

        for requirement in requirements:
            requirement_tokens = self._tokenize(requirement)
            expanded_requirement_tokens = self._expand(requirement_tokens)
            best_overlap, best_resume_highlight = self._best_overlap(
                requirement_tokens,
                expanded_requirement_tokens,
                resume_token_sets,
            )
            match_strength = round(best_overlap, 2)

            if best_overlap >= 0.22:
                matched.append(requirement)
                reason = (
                    f"Matched: '{requirement}' is supported by resume evidence "
                    f"with match strength {match_strength}."
                )
                status = "matched"
            else:
                missing.append(requirement)
                reason = (
                    f"Missing: '{requirement}' is not strongly supported by the current resume text."
                )
                status = "missing"

            rationale.append(reason)
            requirement_evidence.append(
                RequirementEvidence(
                    requirement=requirement,
                    status=status,
                    best_resume_highlight=best_resume_highlight,
                    match_strength=match_strength,
                    reason=reason,
                )
            )

        coverage = len(matched) / max(len(requirements), 1)
        bonus = min(len(resume_highlights), 6) * 3
        score = min(95, round((coverage * 80) + bonus))

        return MatchReport(
            score=score,
            matched_requirements=matched,
            missing_requirements=missing,
            rationale=rationale,
            requirement_evidence=requirement_evidence,
        )

    def _best_overlap(
        self,
        requirement_tokens: set[str],
        expanded_requirement_tokens: set[str],
        resume_token_sets: list[tuple[str, set[str], set[str]]],
    ) -> tuple[float, str | None]:
        if not requirement_tokens or not resume_token_sets:
            return 0.0, None

        best_score = 0.0
        best_direct_hits = 0
        best_support_units = 0
        best_resume_highlight: str | None = None
        for resume_highlight, resume_tokens, expanded_resume_tokens in resume_token_sets:
            direct_hits = len(requirement_tokens & resume_tokens)
            support_units = self._count_support_units(requirement_tokens, resume_tokens)
            supported_overlap = support_units / max(len(requirement_tokens), 1)
            expanded_overlap = len(expanded_requirement_tokens & expanded_resume_tokens) / max(
                len(expanded_requirement_tokens),
                1,
            )
            score = (supported_overlap * 0.75) + (expanded_overlap * 0.25)

            if support_units < 2:
                score = min(score, 0.17)

            is_better_score = score > best_score
            is_better_tie = (
                score == best_score
                and (direct_hits, support_units) > (best_direct_hits, best_support_units)
            )

            if is_better_score or is_better_tie:
                best_score = score
                best_direct_hits = direct_hits
                best_support_units = support_units
                best_resume_highlight = resume_highlight
        return best_score, best_resume_highlight

    def _count_support_units(self, requirement_tokens: set[str], resume_tokens: set[str]) -> int:
        direct_hits = requirement_tokens & resume_tokens
        support_units = len(direct_hits)

        for group in self.SYNONYM_GROUPS:
            requirement_group_hits = requirement_tokens & group
            resume_group_hits = resume_tokens & group
            direct_group_hits = direct_hits & group

            if requirement_group_hits and resume_group_hits and not direct_group_hits:
                support_units += min(len(requirement_group_hits), len(resume_group_hits))
            elif requirement_group_hits and resume_group_hits:
                support_units += max(
                    0,
                    min(len(requirement_group_hits), len(resume_group_hits))
                    - len(direct_group_hits),
                )

        return support_units

    def _tokenize(self, text: str) -> set[str]:
        lowered = text.lower()
        tokens = set(re.findall(r"[A-Za-z][A-Za-z0-9+#-]*", lowered))
        tokens |= self._extract_known_phrases(lowered)
        return {token for token in tokens if token not in self.STOP_WORDS}

    def _expand(self, tokens: set[str]) -> set[str]:
        expanded = set(tokens)
        for group in self.SYNONYM_GROUPS:
            if expanded & group:
                expanded |= group
        return expanded

    def _extract_known_phrases(self, lowered_text: str) -> set[str]:
        phrases: set[str] = set()
        for group in self.SYNONYM_GROUPS:
            for phrase in group:
                if self._should_scan_as_phrase(phrase) and phrase in lowered_text:
                    phrases.add(phrase)
        return phrases

    def _should_scan_as_phrase(self, phrase: str) -> bool:
        return any("\u4e00" <= char <= "\u9fff" for char in phrase) or any(
            char.isdigit() for char in phrase
        )
